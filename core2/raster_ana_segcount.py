    ...
    def load_model(self):
        ...
        if not self.config.change_input_size:
            ...
            self.model = UNet(
                ...
                self.config.input_label_channel,
...
def predict_using_model_segcount_fi(
    model,
    batch,
    batch_pos,
    maskseg,
    ...
    for i in range(len(batch_pos)):
        ...
        if upsample and downsave:  # no preserve range
            ...
            p = resize(
                p[:, :],
                (int(p.shape[0] / upscale), int(p.shape[1] / upscale)),
                preserve_range=True,
            )
            c = resize(
                c[:, :],
                (int(c.shape[0] / upscale), int(c.shape[1] / upscale)),
                preserve_range=True,
            )
            if rescale_values:
                # rescale values
                p = (
                    p
                    * (p.shape[0] / float(p.shape[0] / upscale))
                    * (p.shape[1] / float(p.shape[1] / upscale))
                )
                c = (
                    c
                    * (c.shape[0] / float(c.shape[0] / upscale))
                    * (c.shape[1] / float(c.shape[1] / upscale))
                )
...
def predict_using_model_chm_fi(
    model,
    batch,
    batch_pos,
    mask,
...
def detect_tree_segcount_fi(
    ...
    if downsave or not upsample:
        ...
        masksegs = np.zeros(
            (int(nrows * upscale), int(nols * upscale)), dtype=np.float32
        )
        maskdenss = np.zeros(
            (int(nrows * upscale), int(nols * upscale)), dtype=np.float32
        )
    ...
    for col_off, row_off in tqdm(offsets):
        ...
        if upsample:
            patch1 = np.zeros(
                (int(height * upscale), int(width * upscale), len(config.channels))
            )
            ...
            patch1 = np.zeros(
                (height, width, len(config.channels))
        ...
        print("temp_im1 shape:", temp_im1.shape)
        print("config.channels:", config.channels)
        ...
        if normalize:
            temp_im1 = image_normalize(
                temp_im1, axis=(0, 1)
            )  # Normalize the image along the width and height i.e. independently per channel

        if upsample:
            patch1[: int(window.height * upscale), : int(window.width * upscale)] = (
                temp_im1
            )
        else:
        ...
        if downsave or not upsample:
            ...
            batch_pos.append(
                (
                    int(window.col_off * upscale),
                    int(window.row_off * upscale),
                    int(window.width * upscale),
                    int(window.height * upscale),
                )
            )
        ...
        if len(batch) == config.BATCH_SIZE:
            ...
            masksegs, maskdenss = predict_using_model_segcount_fi(
                model,
                batch,
                batch_pos,
                masksegs,
...
def detect_tree_rawtif_fi(
    ...
    for col_off, row_off in tqdm(offsets):
        ...
        if config.addndvi:
            NDVI = (
                temp_im[:, :, -1].astype(float) - temp_im[:, :, 0].astype(float)
            ) / (temp_im[:, :, -1].astype(float) + temp_im[:, :, 0].astype(float))
        ...
        if len(channels) > 3:
            ...
            if config.gbnorm:  # all bands
                ...
                temp_im = temp_im / 255
                temp_im = (
                    temp_im - np.array([[0.317, 0.350, 0.321, 0.560, 0]])
                ) / np.array([[0.985, 0.895, 0.703, 1.107, 1]])
        ...
        # elif len(config.channels) == 3: # 3 bands
            ...
            len(channels) == 3
        ):  # 3 bands, by default this is for FI so only rg+NIR bands
            if (
                channels[0] == 1
            ):  # to confirm: this is the case for FI, first band is green
                if config.robustscale:  # DK rebust scale
                    ...
                    temp_im = np.array([0, 0, 139]) + (
                        (temp_im - mins) / (maxs - mins)
                    ) * np.array([255, 255, 255 - 139])
                    ...
                    temp_im = (temp_im - np.array([[73.0, 72.0, 145.0]])) / np.array(
                        [[113.0 - 73.0, 96.0 - 72.0, 182.0 - 145.0]]
                    )
                ...
                if config.gbnorm:  # 3 bands DK gb norm from training set
                    ...
                    # temp_im = (temp_im - np.array([[0.37, 0.34, 0.63]]))/ np.array([[0.74, 0.52, 0.81]])
                    temp_im = (temp_im - np.array([[0.350, 0.321, 0.560]])) / np.array(
                        [[0.895, 0.703, 1.107]]
                    )
                ...
                if config.gbnorm_FI:
                    ...
                    temp_im = (temp_im - np.array([[0.253, 0.300, 0.321]])) / np.array(
                        [[0.122, 0.118, 0.127]]
                    )
                ...
                if config.gbnorm:  # 3 bands DK gb norm from training set
                    ...
                    temp_im = temp_im / 255
                    temp_im = (temp_im - np.array([[0.317, 0.350, 0.321]])) / np.array(
                        [[0.985, 0.895, 0.703]]
                    )
                    # print(temp_im.mean(axis = (0, 1)))
        ...
        if upsample:
            ...
            batch_pos.append(
                (
                    int(window.col_off / 2),
                    int(window.row_off / 2),
                    int(window.width / 2),
                    int(window.height / 2),
                )
            )
        ...
        # batch_pos.append((int(window.col_off/2), int(window.row_off/2), int(window.width/2), int(window.height/2)))
        if len(batch) == config.BATCH_SIZE:
            mask = predict_using_model_chm_fi(
                model,
                batch,
                batch_pos,
                mask,
...
def predict_ready_run(
    config, all_files, model_segcount, model_chm, output_dir, eva=0, th=0.5, rgb2gray=0
):
    counter = 1
    th = th
    counts = {}
    ...
    for fullPath, filename in tqdm(all_files):
        outputFile = os.path.join(
            output_dir, filename[:-4] + config.output_suffix + config.output_image_type
        )
        print(outputFile)
        ...
        if not os.path.exists(outputFile) or not os.path.exists(outputFile2):
            ...
            with rasterio.open(fullPath) as img:
                ...
                if config.segcountpred:
                    ...
                    detectedMaskSeg, detectedMaskDens, detectedMeta = (
                        detect_tree_segcount_fi(
                            config,
                            model_segcount,
                            img,
                            width=config.WIDTH,
                            ...
                            auxData=config.aux_data,
                            singleRaster=config.single_raster,
                            multires=config.multires,
                            upsample=config.upsample,
                            downsave=config.downsave,
                    ...
                    writeMaskToDisk(
                        ...
                        detectedMeta,
                        outputFile,
                        image_type=config.output_image_type,
                        output_shapefile_type=config.output_shapefile_type,
                    ...
                    writeMaskToDisk(
                        detectedMaskDens,
                        detectedMeta,
                        outputFile.replace("seg.tif", "density.tif"),
                        image_type=config.output_image_type,
                        output_shapefile_type=config.output_shapefile_type,
                ...
                if config.chmpred:
                    ...
                    writeMaskToDiskChm(
                        detectedMaskChm,
                        detectedMetaChm,
                        outputFile.replace("seg.tif", "chm.tif"),
                        image_type=config.output_image_type,
                        write_as_type=config.output_dtype_chm,
                        scale=0,
                    )
...
def writeMaskToDiskChm(
    detected_mask, detected_meta, wp, image_type, write_as_type="float32", scale=1
):
    # Convert to correct required before writing
